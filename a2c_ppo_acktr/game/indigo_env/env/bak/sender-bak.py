    ...
    def run(self):
        ...
        while(1):
            ...
            if self.step_cnt != step_cnt:
                ...
                self._write_text += 'env_thread id: %s |IN step_cnt err: thread- %s, now-%s \n' % (self.env_id, self.step_cnt, step_cnt)
...
# ==============================================================================
    ...
    def sample_action(self, state, new_add_state):
        ...
        # self.take_action(expert_action)
        
        if self.debug:
            now_time = time.time()
            self.log_start_time = time.time()
            self.sampling_file.write('id: %s | RTTs: %.2f | RTTd: %.2f | next_ack: %s | seq: %s \n'  \

...
def create_env(task_index):
    ...
    best_cwnd_map = yaml.safe_load(open(best_cwnds_file))
    ...
    if task_index == 0:
        ...
        cartesian = [(b, d) for b in bandwidth for d in delay]
        # bandwidth, delay = cartesian[task_index - 26]# error
        bandwidth, delay = cartesian[task_index - 22]
    ...
    # elif task_index == 30:
    #     trace_path = path.join(project_root.DIR, 'env', '0.57mbps-poisson.trace')
    #     mm_cmd = 'mm-delay 28 mm-loss uplink 0.0477 mm-link %s %s --downlink-queue=droptail --downlink-queue-args=packets=14' % (trace_path, trace_path)
    #     best_cwnd = 5
    else:
        raise Exception('task index error!')

    # env = Environment(mm_cmd)
    # env.best_cwnd = best_cwnd

    return mm_cmd, best_cwnd
